...
const UserAllProducts = () => {
  ...
  const fetchHighestBid = async (product) => {
    try {
      const response = await getHighestBid(product.id);
      const highestBid = response.data?.bidAmount || 0;
      setCurrentHighestBid(highestBid > product.price ? highestBid : product.price);
    } catch (error) {
      console.error('Error fetching highest bid:', error);
      setCurrentHighestBid(product.price);
    }
  };
  ...
  const handlePlaceBid = async (e) => {
    ...
    try {
      ...
      if (response.data.startsWith('200::')) {
        ...
        fetchProducts();
        fetchHighestBid(selectedProduct);
      } else {
  ...
  const getImageUrl = (product) => {
    ...
    if (product.photoUrl.startsWith('http')) {
      return product.photoUrl.replace('8080', '8082');
    }
            ...
            <Grid item key={product.id} xs={12} sm={6} md={4}>
              <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                ...
                <CardContent>
                  ...
                  <Typography variant="h6" color="primary" sx={{ mb: 2, mt: 1 }}>
